...
const ALL_PRODUCTS: ProductCard[] = [
  ...
  },
  {
    ...
    collections: [{ name: "Swim", slug: "swim" }, { name: "Sale", slug: "sale" }],
  },
  {
    ...
    featured: true, hasDiscount: false,
    collections: [{ name: "Bodywear", slug: "bodywear" }, { name: "New Arrivals", slug: "new-arrivals" }],
  },
  {
    ...
    collections: [{ name: "Swim", slug: "swim" }, { name: "New Arrivals", slug: "new-arrivals" }],
  },
  {
  ...
  {
    id: "10", name: "Monaco Rib Bodysuit", slug: "monaco-rib-bodysuit",
    images: ["https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=85"],
    basePrice: 5900, lowestPrice: 4130, highestPrice: 5900,
    featured: false, hasDiscount: true,
    collections: [{ name: "Bodywear", slug: "bodywear" }, { name: "Sale", slug: "sale" }],
  },
];

interface PageProps {
  params:       Promise<{ category: string }>;
  searchParams: Promise<{ sort?: string; price?: string }>;
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { category } = await params;
  return { title: COLLECTION_NAMES[category] ?? category };
}
...
export default async function CategoryPage({ params, searchParams }: PageProps) {
  ...
  const name         = COLLECTION_NAMES[category] ?? category;

  let products = ALL_PRODUCTS.filter((prod) =>
    prod.collections.some((c) => c.slug === category)
  );
  ...
  return (
    <>
      <Suspense>
        <ShopHeader title={name} count={products.length} />
      </Suspense>
